...
from auth_token import login, initiate_signup, complete_signup, STARTUP_WARNINGS
...
if st.session_state.get("logged_in"):
    # Always go through profile which loads from DB and routes correctly
...
if action == "login":
    ...
    if u and p:
        ...
        if ok:
            ...
            # ── Load profile + plan from DB immediately on login ──────────────
            _db_err = None
            try:
                ...
                if existing and existing.get("days") and len(existing["days"]) > 0:
                    ...
                    for d in existing["days"]:
                        # workout_json is already parsed by get_active_plan
                        structured.append({
                    ...
                    # Plan ready — go straight to dashboard
                    st.switch_page("pages/2_Dashboard.py")
            except Exception as _e:
                _db_err = str(_e)
                import traceback; traceback.print_exc()
            # No plan or DB error — go to profile
            st.session_state._plan_checked = True
            if _db_err:
...
# ══════════════════════════════════════════════════════════════════════════════
# LANDING HERO  (page_mode == "landing")
# ══════════════════════════════════════════════════════════════════════════════
landing_html = (
    "<div class=\'lhero\'>"
...
<div class="legal" style="margin-top:16px">Your data is encrypted and never shared with third parties.</div>"""
    )
...
/* ━━ MOSAIC — full screen grid behind everything ━━ */
.mosaic{{
...
/* ━━ OVERLAY — radial vignette so tiles show through but text is readable ━━ */
.overlay{{
...
function goGetStarted(){{setTimeout(function(){{window.location.href='?action=go_signup';}},50);}}
function goHome(){{setTimeout(function(){{window.location.href='?action=go_home';}},50);}}
